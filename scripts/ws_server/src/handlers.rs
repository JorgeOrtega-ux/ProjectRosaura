...
async fn handle_socket(mut socket: WebSocket, canvas_id: String, ticket: String, state: AppState) {
    ...
    for code in codes {
        ...
        if is_in_room {
            ...
            if is_owner {
                info!("Owner disconnected from session {}. Starting grace period of 15 seconds.", code);
                let state_for_grace = state.clone();
                let grace_code = code.clone();
                let grace_uid = uid_str.clone();
                
                if let Some((_, existing)) = state_for_grace.grace_sessions.remove(&grace_code) {
                    existing.abort();
                }
                
                let task = tokio::spawn(async move {
                    tokio::time::sleep(tokio::time::Duration::from_secs(15)).await;
                    info!("Grace period expired for session {}. Cleaning up resources.", grace_code);
                    ...
                    if let Ok(mut c) = state_for_grace.redis_pool.get().await {
                        ...
                        if !grace_uid.is_empty() {
                            let _: () = c.del(format!("live_share:user_{}", grace_uid)).await.unwrap_or(());
                        }
                    }
                ...
                state.grace_sessions.insert(code.clone(), task);
            } else {
...
pub async fn support_ws_handler(
    ...
    let agent_id = params.get("agent_id").and_then(|v| v.parse::<i32>().ok());
    let is_admin = session_uuid == "admin_console";

    ws.on_upgrade(move |socket| handle_support_socket(socket, session_uuid, client_id, agent_id, is_admin, state))
}
...
async fn handle_support_socket(
    ...
    if is_admin {
        ...
        if let Some(aid) = agent_id {
            state.admin_agents.entry(aid).or_default().insert(connection_id.clone());
            let db_pool = state.db_pool.clone();
            let redis_pool = state.redis_pool.clone();
            tokio::spawn(async move {
                let _ = db::set_support_agent_online(&db_pool, aid).await;
                let count = db::get_support_available_agents_count(&db_pool).await.unwrap_or(1);
                let payload = serde_json::json!({
                    "type": "support_event",
                    "event": "support_availability_changed",
                    "data": {
                        "is_online": count > 0,
                        "available_agents": count
                    }
                }).to_string();
                helpers::publish_redis_event(&redis_pool, "support:events", &payload).await;
            });
        }
        info!("[SupportWS] Admin Live Console connected (ID: {}, Agent: {:?}). Total admin conns: {}", connection_id, agent_id, state.admin_support_conns.len());
        ...
        let mut welcome_obj = serde_json::json!({
            "type": "support_connected",
            "role": if session_uuid == "presence" { "presence" } else { "session" },
            "session_uuid": &session_uuid,
            "connection_id": &connection_id
        });

        if session_uuid == "presence" {
            let count = db::get_support_available_agents_count(&state.db_pool).await.unwrap_or(0);
            welcome_obj["is_online"] = serde_json::json!(count > 0);
            welcome_obj["available_agents"] = serde_json::json!(count);
        }

        let welcome = welcome_obj.to_string();
        let _ = tx.send(OutboundMessage::Text { payload: welcome, exclude_connection: None }).await;
    }
                    ...
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        match msg_type {
                            "ping" | "support_heartbeat" => {
                                ...
                                if is_admin {
                                    if let Some(aid) = agent_id_clone {
                                        let db_pool = state_clone.db_pool.clone();
                                        tokio::spawn(async move {
                                            let _ = db::update_support_agent_heartbeat(&db_pool, aid).await;
                                        });
                                    }
                                }
                            }
    ...
    if is_admin {
        ...
        if let Some(aid) = agent_id {
            let has_remaining = if let Some(mut conns) = state.admin_agents.get_mut(&aid) {
            ...
            if !has_remaining {
                state.admin_agents.remove(&aid);
                let db_pool = state.db_pool.clone();
                let redis_pool = state.redis_pool.clone();
                tokio::spawn(async move {
                    let _ = db::set_support_agent_offline(&db_pool, aid).await;
                    let count = db::get_support_available_agents_count(&db_pool).await.unwrap_or(0);
                    let payload = serde_json::json!({
                        "type": "support_event",
                        "event": "support_availability_changed",
                        "data": {
        ...
        info!("[SupportWS] Admin Live Console disconnected (ID: {}, Agent: {:?}). Remaining admin conns: {}", connection_id, agent_id, state.admin_support_conns.len());
    } else {
