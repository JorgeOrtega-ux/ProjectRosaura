...
pub async fn set_support_agent_online(db: &MySqlPool, agent_id: i32) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let db_support = env::var("DB_SUPPORT_NAME").unwrap_or_else(|_| "db_support".to_string());
    let query = format!(
        "UPDATE `{}`.`support_agent_status` SET status = 'online', last_heartbeat = NOW() WHERE agent_id = ?",
        db_support
    );
    let res = sqlx::query(&query).bind(agent_id).execute(db).await?;
    if res.rows_affected() == 0 {
        let insert_query = format!(
            "INSERT INTO `{}`.`support_agent_status` (agent_id, status, level, max_concurrent_chats, last_heartbeat) VALUES (?, 'online', 'l1', 3, NOW()) ON DUPLICATE KEY UPDATE status = 'online', last_heartbeat = NOW()",
            db_support
        );
        sqlx::query(&insert_query).bind(agent_id).execute(db).await?;
    }
    Ok(())
}

pub async fn update_support_agent_heartbeat(db: &MySqlPool, agent_id: i32) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let db_support = env::var("DB_SUPPORT_NAME").unwrap_or_else(|_| "db_support".to_string());
    let query = format!(
        "UPDATE `{}`.`support_agent_status` SET last_heartbeat = NOW() WHERE agent_id = ?",
        db_support
    );
    sqlx::query(&query).bind(agent_id).execute(db).await?;
    Ok(())
}

pub async fn set_support_agent_offline(db: &MySqlPool, agent_id: i32) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let db_support = env::var("DB_SUPPORT_NAME").unwrap_or_else(|_| "db_support".to_string());
    let query = format!(
        "UPDATE `{}`.`support_agent_status` SET status = 'offline', last_heartbeat = NOW() WHERE agent_id = ?",
        db_support
    );
    sqlx::query(&query).bind(agent_id).execute(db).await?;
    Ok(())
...
pub async fn get_support_available_agents_count(db: &MySqlPool) -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
    let db_support = env::var("DB_SUPPORT_NAME").unwrap_or_else(|_| "db_support".to_string());
    let query = format!(
        "SELECT COUNT(*) as count FROM `{}`.`support_agent_status` WHERE status IN ('online', 'busy') AND last_heartbeat >= DATE_SUB(NOW(), INTERVAL 5 MINUTE)",
        db_support
    );
