...
class AdminController extends BaseController {
    ...
    public function get_monetization_config($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            return $this->respond($this->adminServices->getMonetizationConfig());
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }

    public function update_monetization_config($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            return $this->respond($this->adminServices->updateMonetizationConfig($input));
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }

    public function reset_monetization_config($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            return $this->respond($this->adminServices->resetMonetizationConfig());
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }

    public function get_campaign_details($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            return $this->respond($this->adminServices->getCampaignDetails($input));
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }

    public function save_campaign($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            $safeInput = [
                'uuid' => $input['uuid'] ?? null,
                ...
                'badge_text' => $input['badge_text'] ?? 'Patrocinado',
                'cta_text' => $input['cta_text'] ?? 'Ver oferta',
                'html_content' => $input['html_content'] ?? null
            ];
            return $this->respond($this->adminServices->saveCampaign($safeInput));
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }

    public function toggle_campaign_active($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            $safeInput = ['uuid' => $input['uuid'] ?? null];
            return $this->respond($this->adminServices->toggleCampaignActive($safeInput));
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }
    ...
    public function delete_campaign($input) {
        try {
            $this->requirePermission(PermissionsConstants::MANAGE_MONETIZATION);
            $safeInput = ['uuid' => $input['uuid'] ?? null];
            return $this->respond($this->adminServices->deleteCampaign($safeInput));
        }
        catch (\Throwable $e) { return $this->handleException($e, __FUNCTION__); }
    }
}