...
class AdminUserPurchasesController {
    constructor() {
        this.api = new ApiService();
        this.abortController = null;
        this.name = 'AdminUserPurchasesController';
        this.container = null;
        this.tbody = null;
    ...
    async init() {
        ...
        if (this.container) {
            this.targetUserUuid = this.container.getAttribute('data-user-uuid');
            this.targetUserId = this.container.getAttribute('data-user-id');
        }
        this.bindEvents();
    ...
    async loadHistory() {
        ...
        try {
            const response = await this.api.post(
                ApiRoutes.Admin.GetUserPurchases, 
                { target_user_uuid: this.targetUserUuid }, 
            ...
            if (response.success && response.data && response.data.length > 0) {
                this.rawItems = response.data.map(item => {
                    ...
                    const description = escapeHTML(item.description || window.__('subscription'));
