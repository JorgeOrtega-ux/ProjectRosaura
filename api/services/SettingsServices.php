...
class SettingsServices {
    ...
    public function updateAvatar($data) {
        ...
        if (move_uploaded_file($file['tmp_name'], $destPath)) {
            ...
            if ($stmt->execute([$newRelPath, $_SESSION['user_id']])) {
                ...
                Logger::security("Avatar de usuario actualizado", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
    ...
    public function deleteAvatar() {
        ...
        if ($stmt->execute([$newRelPath, $_SESSION['user_id']])) {
            $this->logProfileChange($_SESSION['user_id'], 'avatar', $oldPic, $newRelPath);
            $_SESSION['user_pic'] = $newRelPath;
            Logger::security("Avatar de usuario eliminado", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
    ...
    public function updateUsername($data) {
        ...
        if ($stmtUpd->execute([$username, $_SESSION['user_id']])) {
            ...
            Logger::security("Nombre de usuario actualizado: {$oldUsername} -> {$username}", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
    ...
    public function requestEmailCode() {
        ...
        if (!$rateCheck['allowed']) {
            Logger::security("Límite de tasa excedido en solicitud de código de cambio de email", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        ...
        if ($insertStmt->execute([$email, $code, $payload, $expiresAt])) {
            ...
            if ($mailer->sendEmailUpdateCode($email, $_SESSION['user_name'], $code)) {
                ...
                Logger::security("Código para actualización de email enviado", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
    ...
    public function verifyEmailCode($data) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            Logger::security("Identidad verificada exitosamente para cambio de email", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
            return ['success' => true, 'message' => 'Identidad verificada. Tienes 15 minutos.'];
        }
        Logger::security("Código de verificación incorrecto en cambio de email", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function updateEmail($data) {
        ...
        if (empty($_SESSION['can_update_email_expires']) || $_SESSION['can_update_email_expires'] < time()) {
            Logger::security("Intento de cambio de email sin identidad verificada", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        ...
        if ($stmtUpd->execute([$email, $_SESSION['user_id']])) {
            ...
            Logger::security("Correo electrónico actualizado: {$oldEmail} -> {$email}", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function verifyCurrentPassword($data) {
        ...
        if (!$rateCheck['allowed']) {
            Logger::security("Límite de tasa excedido en verificación de contraseña actual", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
            return ['success' => false, 'message' => $rateCheck['message']];
        }
        ...
        if ($user && password_verify(trim($data['current_password'] ?? ''), $user['password'])) {
            ...
            Logger::security("Contraseña actual verificada exitosamente", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
            return ['success' => true, 'message' => 'Identidad verificada.'];
        }
        $this->rateLimiter->record('verify_current_password', 5, 15);
        Logger::security("Verificación de contraseña actual fallida", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function updatePassword($data) {
        ...
        if (empty($_SESSION['can_change_password_expires']) || $_SESSION['can_change_password_expires'] < time()) {
            Logger::security("Intento de actualización de contraseña sin verificación previa", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        ...
        if ($stmt->execute([password_hash($newPassword, PASSWORD_BCRYPT), $_SESSION['user_id']])) {
            ...
            Logger::security("Contraseña de cuenta actualizada manualmente", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function deleteAccount($data) {
        ...
        if ($user && password_verify(trim($data['password'] ?? ''), $user['password'])) {
            if ($this->pdo->prepare("UPDATE users SET user_status = 'deleted' WHERE id = ?")->execute([$_SESSION['user_id']])) {
                ...
                Logger::security("Cuenta de usuario marcada como eliminada", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
                session_unset(); session_destroy();
                return ['success' => true, 'message' => 'Tu cuenta ha sido eliminada.'];
            }
        }
        Logger::security("Intento fallido de eliminación de cuenta (Contraseña incorrecta)", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function generate2faSetup() {
        ...
        Logger::security("Solicitud de generación de credenciales 2FA iniciada", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id']]);
    ...
    public function enable2fa($data) {
        ...
        if ($ga->verifyCode($secret, $code, 2)) {
            ...
            if ($stmt->execute([$secret, json_encode($codes), $_SESSION['user_id']])) {
                ...
                Logger::security("Autenticación de Dos Factores (2FA) habilitada", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
                return ['success' => true, 'message' => 'Activado con éxito.', 'recovery_codes' => $codes];
            }
        }
        Logger::security("Fallo al intentar habilitar 2FA (Código incorrecto)", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
    ...
    public function disable2fa($data) {
        ...
        if ($user && password_verify(trim($data['password'] ?? ''), $user['password'])) {
            ...
            Logger::security("Autenticación de Dos Factores (2FA) deshabilitada", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
            return ['success' => true, 'message' => 'Desactivado.'];
        }
        Logger::security("Intento fallido de deshabilitar 2FA (Contraseña incorrecta)", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        return ['success' => false, 'message' => 'Contraseña incorrecta.'];
    }
    ...
    public function revokeDevice($data) {
        ...
        if ($stmt->execute([$data['device_id'] ?? null, $_SESSION['user_id']])) {
            Logger::security("Sesión de dispositivo revocada manualmente", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'device_id' => $data['device_id']]);
    ...
    public function revokeAllDevices() {
        if (!isset($_SESSION['user_id'])) return ['success' => false, 'message' => 'Sesión no válida.'];
        $currentSelector = isset($_COOKIE['remember_token']) ? explode(':', $_COOKIE['remember_token'])[0] : '';
        if ($this->pdo->prepare("DELETE FROM auth_tokens WHERE user_id = ? AND selector != ?")->execute([$_SESSION['user_id'], $currentSelector])) {
            Logger::security("Todas las demás sesiones de dispositivos fueron revocadas", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        ...
        if ($user = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if (password_verify(trim($data['password'] ?? ''), $user['password'])) {
                ...
                if ($this->pdo->prepare("UPDATE users SET two_factor_recovery_codes = ? WHERE id = ?")->execute([json_encode($codes), $_SESSION['user_id']])) {
                    Logger::security("Códigos de recuperación de 2FA regenerados", Logger::LEVEL_INFO, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
                    return ['success' => true, 'message' => 'Códigos generados.', 'recovery_codes' => $codes];
                }
            }
        }
        Logger::security("Intento fallido de regenerar códigos 2FA (Contraseña incorrecta)", Logger::LEVEL_WARNING, ['user_id' => $_SESSION['user_id'], 'ip' => Utils::getIpAddress()]);
        return ['success' => false, 'message' => 'Contraseña incorrecta.'];
    }
